...
class TableController extends BaseController {
	...
	public function export($categoryId,$khetId,$methodId,$foundAtId,$locationId,$startDate,$endDate)
	{
	...
	function convertAdminTableToPDF($categoryId,$khetId,$methodId,$foundAtId,$locationId,$startDate,$endDate){
		...
		{	
			...
			//foreach($table as $temp){
			//	$temp->location_id = Location::find($temp->location_id)->name;
			//}
			...
			//$pdf = PDF::loadView('table.test', array('table' => $table, 'total'=>$total));
			//return $pdf->stream();
			
			//return Redirect::to(asset('mpdf60/export.php?table='.json_encode($table).'&total='.json_encode($total)));

			if(count($table) < 300)
			{
				return View::make('table/get_report_table',compact('table','total'));
			}
			else{
				return View::make('table/table_warning');
			}
	...
	function convertLocationTableToPDF($startDate,$endDate){
		
		
		$startDate= date("Y-m-d", strtotime(Report::convertYearBtoC($startDate)));	
		$endDate= date("Y-m-d", strtotime(Report::convertYearBtoC($endDate)));	
		$table = ReportSummary::where('found_date','>=',$startDate)
		->where('found_date','<=',$endDate)
		->where('location_id','=',Auth::user()->location->id)
		->get();
		$total = Report::getTotal(Auth::user()->location->id, $startDate, $endDate);
		$filename = 'location_'.Auth::user()->location->id.'.pdf';
		//echo $startDate;
		if(count($table) < 100)
		{
			return View::make('table/get_report_table_location',compact('table','total','filename'));
		}
		else{
			return View::make('table/table_warning');
		}
